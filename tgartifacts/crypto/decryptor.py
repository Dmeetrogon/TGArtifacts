...
class Decryptor:
    ...
    def decrypt_TDF_file(self, encrypted_data: bytes) -> bytes:
    ...
    def decrypt_media_cache_directory(self, cache_path: Path, output_dir: Path) -> Dict[str, int]:
        """Extract and decrypt all TDEF files from media cache.

        Args:
            cache_path: Path to cached and encrypted directory
...
def decrypt_TDF_file_legacy(encrypted_data: bytes, auth_key: bytes) -> bytes:
